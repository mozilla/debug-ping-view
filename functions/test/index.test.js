...
describe('Cloud Functions', () => {
  ...
  describe('debugPing', () => {
    ...
    it('should store debug pings in Firestore', (done) => {
      ...
      clientDocStub.withArgs('b7b6b596-3d06-4dd5-9022-071ac9aeeccb').returns(clientDocRef);
      ...
      const res = {
          ...
          expect(clientUpdateQuery.args[1]).to.have.all.keys('lastActive', 'debugId', 'geo', 'os', 'app_name');
          const pingUpdateQuery = batchSetStub.getCall(1);
          assert.equal(pingUpdateQuery.args[0], pingDocRef);
          expect(pingUpdateQuery.args[1]).to.have.all.keys('clientId', 'payload', 'pingType', 'addedAt');
